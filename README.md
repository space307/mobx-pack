...
Mobx-pack - это маленькая библиотека, которая решает некоторые проблемы DI (инъекция зависимостей) больших приложений, которые
возникают у разработчика при использовании mobx.

## Задачи, которые решает mobx-pack

- разрешение зависимостей одного сервиса от другого, как статически так и динамически
...
- `React.Component`
- `options` (опционально)
  1. helper(function) - функция, принимает объект с сервисами и props, возвращает props для декорируемого компонента
