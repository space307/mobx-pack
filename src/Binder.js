...
class Binder implements BinderInterface {
  ...
  depsList: {
    [key: $Values<typeof CALLBACK_NAME>]:
  ...
  applyCallback(
    ...
    service: *,
    callbackType: $Values<typeof CALLBACK_NAME>,
  ): void {
    const funcToCall = this.parseCallback(callback, service);
    ...
    if (funcToCall) {
      ...
      this.emitter.emit(EMITTER_EVENT.CALLBACK_CALLED, {
        bindAs, callbackType, callback, serviceList,
      });
    } else {
   ...
   */
  lookOverCallback(
    callbackSetList: ?Array<*>,
    cb: (serviceName: ServiceConfigBindAsType) => void,
  ): void {
    if (callbackSetList) {
      ...
      if (importData && importData[from.bindAs]) {
        each(importData[from.bindAs], (toVarName, fromVarName) => {
          ...
          if (toVarName in to.service) {
            this.showMessage(
              `Trying create link from "${from.bindAs}.${fromVarName}"
  ...
  callApi(
    serviceName: ServiceConfigBindAsType,
    actionName: ServiceConfigBindAsType,
    initiator: ServiceConfigBindAsType,
    ...arg: Array<*>
  ) {
    if (process.env.NODE_ENV === 'test') {
      return;
